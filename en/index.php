...
<?php endif; ?>
        ...
        <div class="about-pillars">
          ...
          <div class="card card--feature reveal reveal--delay-3">
            ...
            <a href="/en/cursos/" class="card__cta" data-i18n="about_card3_cta">View KA1 courses</a>
          </div>
        </div>
      </div>
    </section>
...
?>
<?php
    // DEBUG (localhost only): ?force_cursos=N to test 0/1/2/3-course states without touching DB.
    // Pads with synthetic stubs when N exceeds real courses, so the layout can be visualized.
    ...
    if ($_is_local && isset($_GET['force_cursos'])) {
        ...
        $_stub_titles = [
            ...
            'Spanish education system for European teachers',
            'Active methodologies and gamification in the classroom',
        ];
        $_stub_dates = [
        ...
        while (count($_proximos) < $_force_n) {
            ...
            $_proximos[] = [
                ...
                'slug'               => 'demo-course-' . ($i + 1),
                'extracto'           => 'Synthetic course used for local layout testing.',
                'precio'             => 480 + ($i * 60),
                'duracion_dias'      => 5,
    ...
    <section class="section" id="courses">
      <div class="container">
        <div class="section__header reveal">
          ...
          <h2>Upcoming editions in Jerez</h2>
          <p>Five days, Europass certificate, KA1 funding — and Jerez as host city. Limited seats per edition.</p>
        </div>
...
<?php endif; ?>
            ...
            <div class="course-hero-card__body">
              <h3 class="course-hero-card__title">
                <a href="/en/cursos/<?= htmlspecialchars($_protag['slug']) ?>/"><?= htmlspecialchars($_protag['titulo']) ?></a>
              </h3>
              <div class="course-hero-card__chips">
                <span class="course-hero-card__chip">
                  ...
                  <?= (int)$_protag['duracion_dias'] ?> days
                </span>
                <span class="course-hero-card__chip">
...
<?php if (!empty($_protag['fecha_inicio']) && !empty($_protag['fecha_fin'])): $_pfi = strtotime($_protag['fecha_inicio']); $_pff = strtotime($_protag['fecha_fin']); ?>
              <div class="course-hero-card__dates">
                <span class="course-hero-card__dates-label">Edition</span>
                <span class="course-hero-card__dates-value"><?= date('j', $_pfi) ?>–<?= date('j', $_pff) ?> <?= date('M Y', $_pfi) ?></span>
              </div>
<?php else: ?>
              <div class="course-hero-card__dates">
                <span class="course-hero-card__dates-label">Edition</span>
                <span class="course-hero-card__dates-value">Upcoming editions 2026</span>
              </div>
<?php endif; ?>
              <div class="course-hero-card__footer">
                <span class="course-hero-card__price"><?= number_format($_protag['precio'], 0, ',', '.') ?> €
                  <small>per teacher, KA1 fundable</small>
                </span>
<?php if ((int)$_protag['plazas_disponibles'] > 0 && (int)$_protag['plazas_disponibles'] <= 4): ?>
                <span class="course-hero-card__spots-low"><?= (int)$_protag['plazas_disponibles'] ?> spots</span>
<?php endif; ?>
                <a href="/en/cursos/<?= htmlspecialchars($_protag['slug']) ?>/" class="btn btn--gold">Book a seat</a>
              </div>
            </div>
          </article>
...
<?php foreach ($_secs as $sec): $_sfi = !empty($sec['fecha_inicio']) ? strtotime($sec['fecha_inicio']) : null; ?>
            <a href="/en/cursos/<?= htmlspecialchars($sec['slug']) ?>/" class="upcoming-card upcoming-card--compact">
...
<?php else: ?>
                  <?= $sec['duracion_dias'] ?> days · <?= htmlspecialchars($sec['ubicacion']) ?>
<?php endif; ?>
                </div>
                <div class="upcoming-card__footer">
                  <span class="upcoming-card__price"><?= number_format($sec['precio'], 0, ',', '.') ?> €</span>
                  <span class="upcoming-card__spots"><?= (int)$sec['plazas_disponibles'] ?> spots</span>
                </div>
              </div>
            </a>
<?php endforeach; ?>
...
<?php else: ?>
        <div class="courses-empty reveal">
          <h3 class="courses-empty__title">2026 editions in preparation</h3>
          <p class="courses-empty__text">We're closing the KA1 calendar for 2026 (February and June in Jerez). Subscribe to the newsletter and we'll let you know as soon as dates and prices are published.</p>
          <a href="#contact" class="btn btn--primary">Notify me by email</a>
        </div>
<?php endif; ?>

        <div class="courses-foot">
<?php if ($_count_total > 0): ?>
          <p class="courses-foot-note"><?= $_count_total ?> <?= $_count_total === 1 ? 'edition scheduled' : 'editions scheduled' ?> · Europass certificate included</p>
<?php endif; ?>
