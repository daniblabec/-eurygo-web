...
if (!$art || ($art['publicado'] != 1 && $preview !== '1')) {
    ...
    $titulo_pagina = 'Artículo no encontrado | EuryGo';
    $meta_description = '';
    $url_canonica = SITE_URL . '/blog/';
    require_once __DIR__ . '/../includes/header.php';
    echo '<main id="main"><div class="container" style="padding: var(--space-2xl) 0; text-align:center;">';
    echo '<h1>Artículo no encontrado</h1><p>El artículo que buscas no existe o no está publicado.</p>';
    echo '<a href="/blog/" class="btn btn--primary">Volver al blog</a></div></main>';
...
$alt_lang_url = $slug_traduccion ? '/en/blog/' . $slug_traduccion . '/' : '/en/blog/';

// Artículos relacionados (misma categoría, máx 3, excluye actual)
...
$url_canonica = SITE_URL . '/blog/' . $art['slug'] . '/';
$es_articulo = true;
...
$cta = cta_por_categoria($art['categoria'], 'es');
    ...
    <div class="container">
      <article class="article">
        <nav class="article__breadcrumb" aria-label="Breadcrumb">
          ...
          <a href="/blog/?categoria=<?= htmlspecialchars($art['categoria']) ?>"><?= nombre_categoria($art['categoria'], 'es') ?></a> <span class="separator">/</span>
          <span><?= htmlspecialchars(truncar($art['titulo'], 50)) ?></span>
        </nav>

        <header class="article__header">
          <span class="article__category-tag <?= clase_categoria($art['categoria']) ?>"><?= nombre_categoria($art['categoria'], 'es') ?></span>
...
<?php endif; ?>
          <div class="article__meta">
            ...
            <span><?= formato_fecha($art['fecha_publicacion'], 'es') ?></span>
...
<?php
$url_articulo    = urlencode('https://www.eurygo.com/blog/' . ($art['slug'] ?? '') . '/');
$titulo_articulo = urlencode($art['titulo'] ?? 'EuryGo Blog');
$whatsapp_texto  = urlencode('Te comparto este artículo de EuryGo: ' . ($art['titulo'] ?? ''));
?>
        <!-- Compartir -->
        ...
        <!-- Newsletter -->
        <?php $idioma_actual = 'es'; include __DIR__ . '/../includes/widget-newsletter.php'; ?>
...
<?php foreach ($relacionados as $rel): ?>
            <article class="blog-card">
              <div class="blog-card__body">
                <h3><a href="/blog/<?= htmlspecialchars($rel['slug']) ?>/"><?= htmlspecialchars($rel['titulo']) ?></a></h3>
                <p class="blog-card__excerpt"><?= htmlspecialchars(truncar($rel['extracto'], 120)) ?></p>
                <a href="/blog/<?= htmlspecialchars($rel['slug']) ?>/" class="blog-card__link">Leer más <svg viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></a>
              </div>
            </article>
<?php endforeach; ?>
          </div>
