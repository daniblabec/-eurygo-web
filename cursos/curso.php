<?php
/**
 * Cursos de Formación KA1 — Detalle de curso (ES)
 * Recibe ?slug=xxx vía .htaccess
 */
...
// Obtener curso en idioma actual
...
// Fallback: si no existe en este idioma, buscar en cualquier idioma y redirigir
if (!$curso) {
    ...
    $titulo_pagina = 'Curso no encontrado | EuryGo';
    $meta_description = '';
    ...
    echo '<h1>Curso no encontrado</h1><p>El curso que buscas no existe o no está disponible.</p>';
    echo '<a href="/cursos/" class="btn btn--primary">Ver todos los cursos</a></div></main>';
    require_once __DIR__ . '/../includes/footer.php';
    exit;
}

// Obtener slug del curso traducido (para selector de idioma)
$slug_traduccion = '';
...
// Fotos del curso (slider). La tabla puede no existir todavía → array vacío.
$curso_fotos = [];
try {
...
// Agrupar programa por día
$dias = [];
foreach ($programa as $act) {
    $dias[$act['dia']][] = $act;
}

// Cargar ediciones abiertas
...
// Usar la primera edición para fechas por defecto si hay ediciones
...
$url_canonica = SITE_URL . '/cursos/' . $curso['slug'] . '/';
...
// URL alternativa para el selector de idioma
$alt_lang_url = $slug_traduccion ? '/en/cursos/' . $slug_traduccion . '/' : '/en/cursos/';
?>
  ...
  <main id="main" class="course-detail" data-alt-lang-url="<?= htmlspecialchars($alt_lang_url) ?>">
    <div class="container">
      ...
      <nav class="course-detail__breadcrumb" aria-label="Breadcrumb">
        ...
        <a href="/cursos/">Cursos</a> <span class="separator">/</span>
...
<?php if (!empty($curso_fotos)): ?>
      <!-- Slider de fotos del curso -->
      <div class="curso-slider" id="slider-<?= $curso['id'] ?>" data-fotos="<?= count($curso_fotos) ?>" aria-label="Galería de fotos del curso">
...
<?php if (count($curso_fotos) > 1): ?>
        <button class="slider-btn slider-prev" type="button" aria-label="Foto anterior">&#8249;</button>
        <button class="slider-btn slider-next" type="button" aria-label="Foto siguiente">&#8250;</button>
        <div class="slider-dots">
<?php foreach ($curso_fotos as $idx => $f): ?>
          <button class="dot<?= $idx === 0 ? ' active' : '' ?>" type="button" aria-label="Foto <?= $idx + 1 ?>"></button>
<?php endforeach; ?>
        </div>
<?php endif; ?>
      </div>
...
<?php
          // Separar la descripción de los bloques OPTIONAL para poder
          // insertar "What is Included" entre medias.
...
<?php foreach ($dias as $dia_num => $actividades): ?>
            <div class="programme__day">
              <div class="programme__day-header" role="button" aria-expanded="<?= $dia_num <= 2 ? 'true' : 'false' ?>" onclick="this.setAttribute('aria-expanded', this.getAttribute('aria-expanded')==='true'?'false':'true'); this.nextElementSibling.style.display = this.getAttribute('aria-expanded')==='true'?'block':'none';">
                ...
                <span class="programme__day-title">Día <?= $dia_num ?> — <?= htmlspecialchars($actividades[0]['titulo']) ?></span>
...
<?php foreach ($actividades as $act): ?>
                <div class="programme__activity <?= $act['tipo'] === 'excursion' ? 'programme__activity--excursion' : '' ?>">
                  ...
                  <span class="programme__activity-tag programme__activity-tag--<?= $act['tipo'] ?>">
                    <?= $act['tipo'] === 'sesion' ? 'Sesión' : ($act['tipo'] === 'excursion' ? 'Excursión' : 'Actividad') ?>
                  </span>
                </div>
<?php endforeach; ?>
              </div>
        ...
        <aside class="course-sidebar">
          <div class="course-sidebar__card">
            <div class="course-sidebar__price">
              <?= number_format($curso['precio'], 0, ',', '.') ?> € <small>/ participante</small>
            </div>
            <div style="font-size:0.78rem; color:#64748B; margin-top:-0.5rem; margin-bottom:0.75rem;">Financiable con fondos Erasmus+ KA1</div>

<?php if ($plazas_disponibles <= 0): ?>
            <div class="course-sidebar__spots course-sidebar__spots--full">Plazas agotadas</div>
<?php elseif ($plazas_disponibles <= 5): ?>
            <div class="course-sidebar__spots course-sidebar__spots--low">¡Solo quedan <?= $plazas_disponibles ?> plazas!</div>
<?php else: ?>
            <div class="course-sidebar__spots"><?= $plazas_disponibles ?> plazas disponibles</div>
<?php endif; ?>
...
<?php   foreach ($ediciones as $ed): ?>
              <li>
                ...
                <span><?= date('d/m/Y', strtotime($ed['fecha_inicio'])) ?> — <?= date('d/m/Y', strtotime($ed['fecha_fin'])) ?> <small style="color:#64748B;">(<?= $ed['plazas_disponibles'] ?> plazas)</small></span>
              </li>
<?php   endforeach; ?>
<?php else: ?>
              <li>
...
<?php endif; ?>
              ...
              <li>
                ...
                <span><?= $curso['plazas'] ?> plazas máximo</span>
              </li>
              <li>
                <svg viewBox="0 0 24 24"><path d="M12 3L1 9l4 2.18v6L12 21l7-3.82v-6l2-1.09V17h2V9L12 3z"/></svg>
                <span>Certificado Europass incluido</span>
              </li>
              <li>
                <svg viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/></svg>
                <span><?= $curso['duracion_dias'] ?> días / <?= $curso['duracion_dias'] - 1 ?> noches</span>
              </li>
            </ul>

<?php if ($plazas_disponibles > 0): ?>
            <a href="#inscripcion" class="btn btn--eu btn--lg" style="width:100%; text-align:center; display:block;">Inscribirme ahora</a>
<?php else: ?>
            <a href="/#contact" class="btn btn--outline btn--lg" style="width:100%; text-align:center; display:block;">Lista de espera</a>
<?php endif; ?>
          </div>
        </aside>
      </div>
...
<?php if ($plazas_disponibles > 0): ?>
      <div class="enrollment-form" id="inscripcion">
        <h2 class="enrollment-form__title">Formulario de inscripción</h2>
        <p class="enrollment-form__subtitle">Rellena el formulario para reservar tu plaza en este curso. Nos pondremos en contacto contigo para confirmar tu inscripción.</p>
...
<?php if (count($ediciones) > 1): ?>
          <div class="enrollment-form__group enrollment-form__group--full">
            <div class="edition-selector">
              <div class="edition-selector__title">Selecciona la edición *</div>
                ...
                <label class="edition-option" onclick="this.querySelector('input').checked=true; document.querySelectorAll('.edition-option').forEach(e=>e.classList.remove('edition-option--selected')); this.classList.add('edition-option--selected');">
                  ...
                  <div class="edition-option__info">
                    ...
                    <div class="edition-option__spots <?= $ed['plazas_disponibles'] <= 3 ? 'edition-option__spots--low' : '' ?>">
                      <?= $ed['plazas_disponibles'] ?> plazas disponibles
                    </div>
                  </div>
                </label>
<?php   endforeach; ?>
...
<?php endif; ?>
          ...
          <div class="enrollment-form__group">
            <label class="enrollment-form__label" for="enroll-nombre">Nombre completo *</label>
          ...
          <div class="enrollment-form__group">
            <label class="enrollment-form__label" for="enroll-telefono">Teléfono</label>
            <input class="enrollment-form__input" type="tel" id="enroll-telefono" name="telefono">
          </div>

          <div class="enrollment-form__group">
            <label class="enrollment-form__label" for="enroll-pais">País *</label>
            <input class="enrollment-form__input" type="text" id="enroll-pais" name="pais" required>
          </div>

          <div class="enrollment-form__group enrollment-form__group--full">
            <label class="enrollment-form__label" for="enroll-organizacion">Centro educativo / Organización</label>
            <input class="enrollment-form__input" type="text" id="enroll-organizacion" name="organizacion">
          </div>

          <div class="enrollment-form__group enrollment-form__group--full">
            <label class="enrollment-form__label" for="enroll-mensaje">Mensaje (opcional)</label>
          ...
          <div class="enrollment-form__rgpd">
            ...
            <label for="enroll-rgpd">Acepto la <a href="/privacidad/" target="_blank">política de privacidad</a> y el tratamiento de mis datos para gestionar la inscripción en este curso. *</label>
          </div>

          <div class="enrollment-form__actions">
            <button type="submit" class="btn btn--eu btn--lg">Enviar inscripción</button>
          </div>
...
<?php endif; ?>
  ...
  document.getElementById('enrollment-form')?.addEventListener('submit', async function(e) {
    ...
    if (!nombre || !email || !pais) {
      ...
      feedback.textContent = 'Por favor, rellena todos los campos obligatorios.';
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      feedback.style.display = 'block';
    ...
    try {
      // Obtener token CSRF fresco antes de enviar
      ...
      if (data.ok) {
        ...
        feedback.className = 'enrollment-form__feedback enrollment-form__feedback--error';
        feedback.textContent = data.error || 'Error al enviar la inscripción.';
      }
    } catch (err) {
      feedback.style.display = 'block';
      feedback.className = 'enrollment-form__feedback enrollment-form__feedback--error';
